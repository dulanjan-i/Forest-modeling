...
main_cohorts_tidy = function(
  ...
  dnot = 5         # dbh with which recruits are "born" in cm
  ...
  for(time.tmp in seq(deltaT,maxT,by=deltaT)) {
    ...
    # Step 5: Calculate Carbon Stocks
    data <- data %>%
      mutate(
        # Calculate height specifically for carbon calculations
        height_carbon = spdata$param1[sp] / (1 + exp(-(spdata$inflection[sp] - dbh) / spdata$steepness[sp])) +
          spdata$param2[sp] / (1 + exp((spdata$inflection[sp] - dbh) / spdata$steepness[sp])),
        
        # Calculate stem volume using the new height_carbon column
        stem_volume = (dbh^as.numeric(spdata$a[sp])) * 
          (height_carbon^as.numeric(spdata$b[sp])) * 
          exp(as.numeric(spdata$c[sp])),
        
        # Calculate aboveground biomass and carbon stocks
        agb = stem_volume * spdata$Wood_Density[sp],
        carbon_above = agb * 0.47,
        carbon_below = carbon_above * 0.3,
  ...
  # Save outputs
  write_csv(all_cohorts_out, paste0(mainfolder, "/all_cohorts_out_with_carbon.csv"))
  write_csv(stats_out, paste0(mainfolder, "/stats_out_with_carbon.csv"))
  write_csv(dbh_dist, paste0(mainfolder, "/dbh_dist_out_with_carbon.csv"))
  
  plot_temp = plot.output(all_cohorts_out, mainfolder)
  ggsave(paste0(mainfolder, "/plot_1.jpg"), plot_temp, width = 10, height = 8)
...
plot.output = function(all_cohorts_out, mainfolder) {
  ...
  p1 = all_cohorts_out %>%
    ...
    ggplot() + 
    geom_line(aes(x = time, y = ba, color = sp), linewidth = 2) + 
